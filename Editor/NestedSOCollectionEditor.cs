...
namespace NestedSO.SOEditor
{
	...
	public class NestedSOCollectionEditor : Editor
	{
		...
		protected void OnEnable()
		{
			...
			Type baseType = NestedSOEditorUtils.GetBaseTypeFromCollection(targetCollection);
		...
		{
			if (_breadcrumbs.Count == 0) _breadcrumbs.Add(targetObj);
		...
		{
			try { if (target != null && !target.Equals(null)) SaveNavigationState(); } catch { }
		}
		...
		private void DrawBreadcrumbs()
		{
			...
			GUIStyle breadcrumbStyle = new GUIStyle(EditorStyles.toolbarButton) { alignment = TextAnchor.MiddleLeft, richText = true };

			for (int i = 0; i < _breadcrumbs.Count; i++)
			{
				var obj = _breadcrumbs[i];
				if (obj == null) continue;
				string name = obj.name.Length > 20 ? obj.name.Substring(0, 17) + "..." : obj.name;

				if (i == _breadcrumbs.Count - 1) GUILayout.Label(name, breadcrumbStyle, GUILayout.MaxWidth(150));
				else
				{
					if (GUILayout.Button(name, breadcrumbStyle, GUILayout.MaxWidth(150))) NavigateToBreadcrumbIndex(i);
		...
		{
			if (item == null) { EditorGUILayout.HelpBox("Item is null or missing.", MessageType.Warning); return; }
			if (!_cachedEditors.TryGetValue(item, out Editor editor)) _cachedEditors[item] = editor = CreateEditor(item);
		...
		private void DrawMassEditInterface()
		{
			...
			if (_massEditExpanded)
			{
				...
				{
					while (iter.NextVisible(false)) if (iter.name != "m_Script") props.Add(iter.name);
				}
				...
				if (newIndex != selectedIndex && newIndex >= 0 && newIndex < props.Count) _massEditSelectedPropertyPath = props[newIndex];
				EditorGUILayout.EndHorizontal();
				...
				if (!string.IsNullOrEmpty(_massEditSelectedPropertyPath))
				{
					...
					if (targetProp != null)
					{
						...
						if (EditorGUI.EndChangeCheck()) representative.ApplyModifiedProperties();

						if (GUILayout.Button("Apply to All", GUILayout.Width(100))) ApplyMassEdit(representative, targetProp);
						EditorGUILayout.EndHorizontal();
					}
				}
			}
		...
		private void DrawSearchResults()
		{
			...
			if (GUILayout.Button("Pop Range", GUILayout.Width(150)))
			{
				...
				{
					collection._RemoveAsset(poppedAsset);
					collection._MarkAsRemovedAsset(poppedAsset);
				});
			...
			GUIStyle highlightStyle = new GUIStyle(EditorStyles.miniLabel) { richText = true };
			...
			for (int i = 0; i < _filteredItems.Count; i++)
			{
				...
				EditorGUILayout.BeginVertical("helpBox");
				{
					EditorGUILayout.BeginHorizontal();
					{
						...
						if (GUILayout.Button("Open", GUILayout.Width(60))) OpenItem(item);
					}
					EditorGUILayout.EndHorizontal();
					...
					if (match.MatchDetails.Count > 0)
					{
						...
						foreach (string detail in match.MatchDetails) GUILayout.Label(detail, highlightStyle);
						EditorGUI.indentLevel--;
					}
				}
				EditorGUILayout.EndVertical();
				...
				if (evt.type == EventType.DragPerform)
				{
					...
					foreach (var obj in DragAndDrop.objectReferences)
					{
						if (obj is ScriptableObject so && AssetDatabase.IsMainAsset(so))
						{
							...
							{
								ScriptableObject clone = NestedSOEditorUtils.PushExternalAsset(so, collection);
								if (clone != null)
								{
		...
		private void OnDrawListElement(Rect rect, int index, bool isActive, bool isFocused)
		{
			...
			if (nestedItem == null) { EditorGUI.LabelField(rect, "Null Item"); return; }

			float editBtnW = 24; float menuBtnW = 24; float padding = 2;
			float nameWidth = rect.width - editBtnW - menuBtnW - (padding * 3);

			string newObjectName = EditorGUI.TextField(new Rect(rect.x, rect.y + 1, nameWidth, EditorGUIUtility.singleLineHeight), nestedItem.name);
			if (nestedItem.name != newObjectName) { nestedItem.name = newObjectName; AssetDatabase.SaveAssets(); EditorUtility.SetDirty(nestedItem); }

			Rect openBtnRect = new Rect(rect.x + nameWidth + padding, rect.y, editBtnW, EditorGUIUtility.singleLineHeight);
			if (IconButton(openBtnRect, "d_scenepicking_pickable_hover")) OpenItem(nestedItem);
			...
			if (GUI.Button(menuBtnRect, menuIcon, new GUIStyle("IconButton")))
			{
				...
				{
					ScriptableObject clone = NestedSOEditorUtils.DuplicateSubAsset(nestedItem, collection);
					collection._AddAsset(clone);
					collection._MarkAsAddedAsset(clone);
					AssetDatabase.SaveAssets();
					EditorUtility.SetDirty(collection);
				...
				{
					if (NestedSOEditorUtils.PopSubAsset(nestedItem, out _))
					{
				...
				menu.AddItem(new GUIContent("Remove"), false, () =>
				{
					collection._RemoveAsset(nestedItem);
					collection._MarkAsRemovedAsset(nestedItem);
					NestedSOEditorUtils.DestroyAsset(nestedItem);
					AssetDatabase.SaveAssets();
					EditorUtility.SetDirty(collection);
				});
		...
		{
			GenericMenu menu = new GenericMenu();
			var types = TypeCache.GetTypesDerivedFrom(_baseType).Where(t => !t.IsAbstract && !t.IsInterface).ToList();
			...
			foreach (var t in types)
			{
				...
				{
					ScriptableObject nestedSOItemInstance = CreateInstance(t);
					nestedSOItemInstance.name = "New " + t.Name;
					NestedSOCollectionBase collection = target as NestedSOCollectionBase;
		...
		private void OnRemoveItem(ReorderableList list)
		{
			...
			{
				NestedSOCollectionBase collection = target as NestedSOCollectionBase;
				collection._RemoveAsset(item);
				collection._MarkAsRemovedAsset(item);
		...
		private void OpenItem(ScriptableObject item)
		{
			_searchString = "";
			GUI.FocusControl(null);
			if (_breadcrumbs.Count == 1) _breadcrumbs.Add(item);
			else
			{
		...
		private void NavigateToBreadcrumbIndex(int index)
		{
			if (index >= 0 && index < _breadcrumbs.Count)
			{
				...
				if (countToRemove > 0) _breadcrumbs.RemoveRange(index + 1, countToRemove);
				SaveNavigationState();
				_searchString = "";
				GUI.FocusControl(null);
			}
		...
		private void PerformSearch()
		{
			...
			for (int i = 0; i < _nestedSOsProperty.arraySize; i++)
			{
				...
				if (item.GetType().Name.ToLowerInvariant().Contains(lowerSearch)) matchDetails.Add($"Type: <color=#FFFF00>{item.GetType().Name}</color>");
				...
				if (iter.NextVisible(true))
				{
					while (iter.NextVisible(false))
					{
						...
						bool valMatch = !string.IsNullOrEmpty(valStr) && valStr.ToLowerInvariant().Contains(lowerSearch);
						bool propNameMatch = iter.displayName.ToLowerInvariant().Contains(lowerSearch);

						if (valMatch || propNameMatch)
						{
							matchDetails.Add(valMatch ? $"{iter.displayName}: <color=#FFFF00>{valStr}</color>" : $"<color=#FFFF00>{iter.displayName}</color>: {valStr}");
						}
					}
				}

				if (nameMatch || matchDetails.Count > 0) _filteredItems.Add(new SearchMatch { Item = item, MatchDetails = matchDetails });
			}
			...
			if (!string.IsNullOrEmpty(_searchString) && _filteredItems.Count > 0)
			{
				...
				if (prop != null) _massEditSelectedPropertyPath = prop.name;
			}
		}
